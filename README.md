...
LangExtract uses `uv` and `pyproject.toml` for dependency management.
...
# Runtime + dev dependencies (tests, lint, type checks)
uv sync --group dev
```
...
LangExtract supports OpenAI models:
...
This fork is maintained for internal product work first. PRs are still welcome
when they are scoped, tested, and aligned with this repository's direction.
...
```bash
uv run tox  # runs ruff + pytest across configured environments
```
...
This project is a derivative fork of Google's LangExtract. Keep both
attribution and license notices intact when redistributing modified versions.

- Upstream citation metadata: `CITATION.cff`
- Upstream DOI: `10.5281/zenodo.17015089`
