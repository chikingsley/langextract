...
def _merge_non_overlapping_extractions(
    all_extractions: list[list[data.Extraction]],
        ...
        for extraction in pass_extractions:
            ...
            if extraction.char_interval is not None:
                for existing_extraction in merged_extractions:
                    if existing_extraction.char_interval is not None and _extractions_overlap(
...
def _extractions_overlap(extraction1: data.Extraction, extraction2: data.Extraction) -> bool:
...
def _document_chunk_iterator(
    ...
    for document in documents:
        ...
        if restrict_repeats and document_id in visited_ids:
            raise exceptions.InvalidDocumentError(f"Document id {document_id} is already visited.")
...
class Annotator:
    ...
    def _annotate_documents_single_pass(
        ...
        **kwargs,
        ...
        per_doc: collections.defaultdict[str, list[data.Extraction]] = collections.defaultdict(list)
        next_emit_idx = 0
        ...
        def _capture_docs(
            ...
            for document in src:
                ...
                if document_id in doc_text_by_id:
                    raise exceptions.InvalidDocumentError(f"Duplicate document_id: {document_id}")
        ...
        try:
            for batch in batch_iter:
                ...
                if show_progress:
                    current_chars = sum(len(text_chunk.chunk_text) for text_chunk in batch)
                ...
                for text_chunk, scored_outputs in zip(batch, outputs, strict=True):
                    ...
                    token_offset = (
                        text_chunk.token_interval.start_index if text_chunk.token_interval else 0
                    )
                    char_offset = (
                        text_chunk.char_interval.start_pos if text_chunk.char_interval else 0
                    )
                    ...
                    if show_progress and text_chunk.char_interval is not None:
                        chars_processed += (
                            text_chunk.char_interval.end_pos - text_chunk.char_interval.start_pos
                        )
    ...
    def _annotate_documents_sequential_passes(
        ...
        **kwargs,
        ...
        logging.info(
            "Starting sequential extraction passes for improved recall with %d passes.",
            extraction_passes,
        )
        ...
        for doc in document_list:
            ...
            if debug:
                total_extractions = sum(len(extractions) for extractions in all_pass_extractions)
                logging.info(
