...
class FormatModeSchema(BaseSchema):
    ...
    def __init__(self, format_type: types.FormatType = types.FormatType.JSON):
        ...
        # Track provider format as a concrete string value.
    ...
    def sync_with_provider_kwargs(self, kwargs: dict[str, Any]) -> None:
        ...
        if "format_type" in kwargs:
            ...
            self._format = "json" if self.format_type == types.FormatType.JSON else "yaml"
        if "format" in kwargs:
            self._format = kwargs["format"]
            self.format_type = (
                types.FormatType.JSON if self._format == "json" else types.FormatType.YAML
            )
