...
# Default tokenizer instance for module-level helper calls.
...
_CJK_PATTERN = regex.compile(r"\p{Is_Han}|\p{Is_Hiragana}|\p{Is_Katakana}|\p{Is_Hangul}")
_NON_SPACED_PATTERN = regex.compile(r"\p{Is_Thai}|\p{Is_Lao}|\p{Is_Khmer}|\p{Is_Myanmar}")
...
_COMMON_SCRIPTS_PATTERN = regex.compile(
    "|".join(rf"(?P<{script}>\p{{Script={script}}})" for script in _COMMON_SCRIPTS)
)
        ...
        for match in regex.finditer(r"\X", text):
            ...
            if current_type is not None and current_type == g_type:
                if current_type == TokenType.WORD:
                    ...
                    if (
                        current_script is _NO_GROUP_SCRIPT
                        or _CJK_PATTERN.match(first_char)
                        or _NON_SPACED_PATTERN.match(first_char)
                    ):
                        should_merge = False
                        ...
                        if current_script == g_script and current_script is not _UNKNOWN_SCRIPT:
                            should_merge = True
                    ...
                    if last_grapheme == grapheme or (
                        len(last_grapheme) >= len(grapheme) and last_grapheme.endswith(grapheme)
                    ):
                        should_merge = True
...
def _is_sentence_break_after_newline(
    ...
    next_token_text = text[next_token.char_interval.start_pos : next_token.char_interval.end_pos]
...
def find_sentence_range(
    ...
    if start_token_index < 0 or start_token_index >= len(tokens):
        raise SentenceRangeError(
            f"start_token_index={start_token_index} out of range. Total tokens: {len(tokens)}."
        )

    i = start_token_index
    while i < len(tokens):
        if tokens[i].token_type == TokenType.PUNCTUATION and _is_end_of_sentence_token(
            text, tokens, i, known_abbreviations
        ):
            end_index = i + 1
