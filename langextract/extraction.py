...
def extract(
    ...
    *,
    ...
    Args:
        ...
        base_url: Endpoint URL for self-hosted or on-prem models.
        ...
        config: Model configuration to use for extraction. Takes precedence over
          model_id and api_key parameters. When both model
    ...
    prompt_template = prompting.PromptTemplateStructured(description=prompt_description or "")
    ...
    try:
        res = resolver.Resolver(format_handler=format_handler, **remaining_params)
    except TypeError as e:
        msg = str(e)
        if "unexpected keyword argument" in msg or "got an unexpected keyword argument" in msg:
            raise TypeError(f"Unknown key in resolver_params; check spelling: {e}") from e
        raise
