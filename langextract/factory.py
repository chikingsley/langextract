...
from langextract.settings import get_settings, resolve_api_key
...
def _kwargs_with_environment_defaults(
    ...
    Each provider declares ENV_API_KEY_NAMES on the class. The factory reads
    those names from settings. No model-id string matching required.

    Args:
      provider_class: The resolved provider class.
    ...
    if "api_key" not in resolved and not resolved.get("vertexai", False):
        env_var_names = getattr(provider_class, "ENV_API_KEY_NAMES", ())
    ...
    if "base_url" not in resolved:
        ...
        if base_url and base_url != "http://localhost:11434":
            # Only inject non-default ollama base_url
            resolved.setdefault("base_url", base_url)
...
def create_model(
    ...
    return_fence_output: Literal[False] = False,
...
def create_model(
    ...
    use_schema_constraints: bool = False,
    fence_output: bool | None = None,
    return_fence_output: Literal[True] = True,
) -> tuple[base_model.BaseLanguageModel, bool]: ...
...
def create_model(
    ...
    kwargs = _kwargs_with_environment_defaults(provider_class, config.provider_kwargs)
...
def _create_model_with_schema(
    ...
    model_id = config.model_id
    kwargs = _kwargs_with_environment_defaults(provider_class, kwargs)
