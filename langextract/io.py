...
def _read_csv(
    ...
    try:
        with filepath.open(encoding="utf-8", newline="") as f:
            ...
            missing = set(column_names) - set(reader.fieldnames)
            if missing:
                raise InvalidDatasetError(
                    f"Invalid dataset file: {filepath} (missing columns: {missing})"
                )
            has_rows = False
            for row in reader:
                has_rows = True
                yield {col: row[col] for col in column_names}
            if not has_rows:
                raise InvalidDatasetError(f"Empty dataset: {filepath}")
    ...
    try:
        with httpx.stream("GET", url, timeout=timeout, follow_redirects=True) as response:
            ...
            if show_progress and total_size > 0:
                progress_bar = progress.create_download_progress_bar(total_size=total_size, url=url)
        ...
        raise httpx.HTTPError(f"Failed to download from {url}: {e!s}") from e
