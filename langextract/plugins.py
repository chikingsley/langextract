...
def _load_class(spec: str) -> type[base_model.BaseLanguageModel]:
    ...
    try:
        ...
        raise ImportError(f"Failed to import provider module '{module_path}': {e}") from e
    ...
    if not isinstance(cls, type) or not issubclass(cls, base_model.BaseLanguageModel):
        ...
        if missing:
            raise TypeError(
                f"{cls} is not a BaseLanguageModel and missing required methods: {missing}"
            )

        logging.warning(
            "Provider %s does not inherit from BaseLanguageModel but appears compatible",
            cls,
        )
...
@functools.cache  # Cache all loaded classes
