...
def create_download_progress_bar(
    ...
    return tqdm.tqdm(
        ...
        unit="B",
        unit_scale=True,
        desc=(f"{BLUE}{BOLD}LangExtract{RESET}: Downloading {GREEN}{url_display}{RESET}"),
        bar_format=(
...
def print_download_complete(char_count: int, word_count: int, filename: str) -> None:
...
def create_save_progress_bar(output_path: str, disable: bool = False) -> tqdm.tqdm:
    ...
    return tqdm.tqdm(
        desc=(f"{BLUE}{BOLD}LangExtract{RESET}: Saving to {GREEN}{filename}{RESET}"),
        unit=" docs",
        disable=disable,
    )
...
def print_save_complete(num_docs: int, file_path: str) -> None:
    ...
    filename = file_path.split("/")[-1]
    print(
        f"{GREEN}✓{RESET} Saved {BOLD}{num_docs}{RESET} documents to {GREEN}{filename}{RESET}",
        flush=True,
    )
...
def print_load_complete(num_docs: int, file_path: str) -> None:
    ...
    filename = file_path.split("/")[-1]
    print(
        f"{GREEN}✓{RESET} Loaded {BOLD}{num_docs}{RESET} documents from {GREEN}{filename}{RESET}",
        flush=True,
    )
...
def get_model_info(language_model: Any) -> str | None:
    ...
    if hasattr(language_model, "base_url"):
        return language_model.base_url
...
def create_pass_progress_bar(total_passes: int, disable: bool = False) -> tqdm.tqdm:
