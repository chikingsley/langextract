...
TLanguageModel = typing.TypeVar("TLanguageModel", bound=base_model.BaseLanguageModel)
...
_entry_keys: set[tuple[str, tuple[str, ...], int]] = set()  # (provider_id, patterns, priority)
...
def _add_entry(
    *,
    ...
    if key in _entry_keys:
        logging.debug(
            "Skipping duplicate registration for %s with patterns %s at priority %d",
            provider_id,
...
def register_lazy(*patterns: str | re.Pattern[str], target: str, priority: int = 0) -> None:
...
def list_providers() -> list[tuple[tuple[str, ...], int]]:
    ...
    """
    return [(tuple(p.pattern for p in entry.patterns), entry.priority) for entry in _entries]
