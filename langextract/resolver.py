...
class Resolver(AbstractResolver):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
          ...
          constraint: Optional parsing constraint.
          format_type: Output format.
          fence_output: Whether to expect fenced outputs.
          strict_fences: Whether to require strict fence parsing.
          require_extractions_key: Whether wrapper key is required.
          attribute_suffix: Suffix used for extraction attribute keys.
        """
        ...
        **kwargs,
        ...
        try:
            ...
            if suppress_parse_errors:
                logging.exception("Failed to parse input_text: %s, error: %s", input_text, e)
                return []
    ...
    def align(
        ...
        **kwargs,
        ...
        if not extractions:
            logging.debug("No extractions found in the annotated text; exiting alignment process.")
            return
        else:
    ...
    def string_to_extraction_data(
        ...
        Delegates to the FormatHandler for actual parsing.
            ...
            for extraction_class, extraction_value in group.items():
                ...
                if attributes_suffix and extraction_class.endswith(attributes_suffix):
                    if not isinstance(extraction_value, (dict, type(None))):
                        ...
                        raise ValueError("Extraction value must be a dict or None for attributes.")
                    continue
                ...
                if not isinstance(extraction_value, (str, int, float)):
                    ...
                    raise ValueError("Extraction text must be a string, integer, or float.")
                ...
                if index_suffix:
                    ...
                    index_value = group.get(index_key, None)
                ...
                if attributes_suffix:
                    ...
                    raw_attributes = group.get(attributes_key, None)
                    if raw_attributes is None:
                        ...
                        raise ValueError("Extraction attributes must be a dict or None.")
...
class WordAligner:
    ...
    def _fuzzy_align_extraction(
        ...
        if best_span and best_ratio >= fuzzy_alignment_threshold:
            ...
            try:
                ...
                logging.exception("Index error while setting intervals during fuzzy alignment.")
                return None
    ...
    def align_extractions(
        ...
        source_tokens = list(_tokenize_with_lowercase(source_text, tokenizer_inst=tokenizer_impl))

        delim_len = len(list(_tokenize_with_lowercase(delim, tokenizer_inst=tokenizer_impl)))
        if delim_len != 1:
        ...
        extraction_tokens = list(
            _tokenize_with_lowercase(
                f" {delim} ".join(
                    extraction.extraction_text for extraction in itertools.chain(*extraction_groups)
                ),
        ...
        for group_index, group in enumerate(extraction_groups):
            ...
            for extraction in group:
                ...
                index_to_extraction_group[extraction_index] = (
                    extraction,
                    group_index,
                )
        ...
        aligned_extraction_groups: list[list[data.Extraction]] = [[] for _ in extraction_groups]
        tokenized_text = (
        ...
        logging.debug("Final aligned extraction groups: %s", aligned_extraction_groups)
