...
def _build_legend_html(color_map: dict[str, str]) -> str:
    ...
    return f'<div class="lx-legend">Highlights Legend: {" ".join(legend_items)}</div>'
...
def _format_attributes(attributes: dict | None) -> str:
    ...
    valid_attrs = {
        key: value for key, value in attributes.items() if value not in (None, "", "null")
    }
...
def _prepare_extraction_data(
    ...
    for i, extraction in enumerate(extractions):
        ...
        attributes_html = (
            f"<div><strong>class:</strong> {html.escape(extraction.extraction_class)}</div>"
        )
        attributes_html += (
            f"<div><strong>attributes:</strong> {_format_attributes(extraction.attributes)}</div>"
        )
...
def _build_visualization_html(
    ...
    start_pos = first_extraction.char_interval.start_pos
      ...
      (function() {{
        ...
        function updateDisplay() {{
          ...
          document.getElementById('entityInfo').textContent =
            (currentIndex + 1) + '/' + extractions.length;
          document.getElementById('posInfo').textContent =
            '[' + extraction.startPos + '-' + extraction.endPos + ']';
          ...
          const currentSpan = document.querySelector(
            '.lx-text-window span[data-idx="' + currentIndex + '"]'
          );
          if (currentSpan) {{
