...
def create_pyproject_toml(base_dir: Path, provider_name: str, package_name: str) -> None:
...
def create_schema(base_dir: Path, provider_name: str, package_name: str) -> None:
...
def create_readme(
    ...
    supported = "\n".join(f"- `{_display(p)}*`: Models matching pattern {p}" for p in patterns)
...
def parse_arguments():
    ...
    parser.add_argument("--no-install", action="store_true", help="Skip installation and testing")
...
def print_summary(
    ...
    print("https://github.com/google/langextract/blob/main/langextract/providers/README.md")


def create_plugin(args: argparse.Namespace, package_name: str, patterns: list[str]) -> Path:
    ...
    create_provider(base_dir, args.provider_name, package_name, patterns, args.with_schema)

    if args.with_schema:
        create_schema(base_dir, args.provider_name, package_name)

    create_test_script(base_dir, args.provider_name, package_name, patterns, args.with_schema)
...
def main():
    ...
    if not args.no_install:
        ...
        if success:
            ...
            print("\n⚠️ Plugin created but tests failed. Please check the implementation.")
            sys.exit(1)
    else:
