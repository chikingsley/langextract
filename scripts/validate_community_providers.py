...
def validate(filepath: Path) -> bool:
    ...
    if start < 0:
        ...
        return False
    if end < 0:
        errors.append("Could not find end marker: <!-- ADD NEW PLUGINS ABOVE THIS LINE -->.")
        print_report(errors, warnings)
        return False
    ...
    for i in range(start + 2, end):
        ...
        if not raw.startswith("|") or not raw.endswith("|"):
            errors.append(f"Line {i + 1}: Not a valid table row (must start and end with '|').")
            continue
        ...
        if not re.fullmatch(PYPI_NORMALIZED, pypi):
            ...
            errors.append(
                f"Line {i + 1}: PyPI package should start with `langextract-` for discoverability."
            )
        ...
        if not desc or len(desc) < MIN_DESC_LEN:
            errors.append(f"Line {i + 1}: Description must be at least {MIN_DESC_LEN} characters.")
