...
class SentenceIterTest(absltest.TestCase):
    def test_basic(self):
        ...
        self.assertEqual(
            tokenizer.TokenInterval(start_index=0, end_index=5),
            sentence_interval,
        )
        self.assertEqual(
        ...
        sentence_interval = next(sentence_iter)
        self.assertEqual(
            tokenizer.TokenInterval(start_index=5, end_index=11),
            sentence_interval,
        )
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, sentence_interval),
        ...
        sentence_interval = next(sentence_iter)
        self.assertEqual(
            tokenizer.TokenInterval(start_index=11, end_index=17),
            sentence_interval,
        )
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, sentence_interval),
...
class ChunkIteratorTest(absltest.TestCase):
    def test_multi_sentence_chunk(self):
        ...
        self.assertEqual(tokenizer.TokenInterval(start_index=0, end_index=11), chunk_interval)
        self.assertEqual(
    ...
    def test_break_sentence(self):
        ...
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(tokenizer.TokenInterval(start_index=0, end_index=3), chunk_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            "This is a",
        )
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(tokenizer.TokenInterval(start_index=3, end_index=5), chunk_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            "sentence.",
        )
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(tokenizer.TokenInterval(start_index=5, end_index=8), chunk_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            "This is a",
        )
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(tokenizer.TokenInterval(start_index=8, end_index=9), chunk_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            "longer",
        )
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(tokenizer.TokenInterval(start_index=9, end_index=11), chunk_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            "sentence.",
        )
    ...
    def test_long_token_gets_own_chunk(self):
        ...
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(tokenizer.TokenInterval(start_index=0, end_index=2), chunk_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            "This is",
        )
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(tokenizer.TokenInterval(start_index=2, end_index=3), chunk_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            "a",
        )
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(tokenizer.TokenInterval(start_index=3, end_index=4), chunk_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            "sentence",
        )
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(tokenizer.TokenInterval(start_index=4, end_index=5), chunk_interval)
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
            ".",
        )
    ...
    77 y o woman in NAD with a h/o CAD, DM2, asthma and HTN on altace.""")
        ...
        chunk_interval = next(chunk_iter).token_interval
        self.assertEqual(
            tokenizer.TokenInterval(start_index=0, end_index=len(tokenized_text.tokens)),
            chunk_interval,
        )
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, chunk_interval),
    ...
    Diltiazem 300 mg qhs \u2013 Ca channel blocker used to control hypertension""")
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, first_chunk.token_interval),
    ...
    renal protective effect""")
        self.assertEqual(
            chunking.get_token_interval_text(tokenized_text, second_chunk.token_interval),
...
class BatchingTest(parameterized.TestCase):
    _SAMPLE_DOCUMENT = data.Document(
        text=("Sample text with numerical values such as 120/80 mmHg, 98.6°F, and 50mg."),
    )
        ...
        (
            ...
            [
                [
                    ...
                    chunking.TextChunk(
                        token_interval=tokenizer.TokenInterval(start_index=10, end_index=14),
                        document=_SAMPLE_DOCUMENT,
                    ),
                    chunking.TextChunk(
                        token_interval=tokenizer.TokenInterval(start_index=14, end_index=19),
                        document=_SAMPLE_DOCUMENT,
                    ),
                    chunking.TextChunk(
                        token_interval=tokenizer.TokenInterval(start_index=19, end_index=22),
                        document=_SAMPLE_DOCUMENT,
                    ),
                ]
            ],
...
class TextAdditionalContextTest(absltest.TestCase):
    ...
    def test_text_chunk_additional_context(self):
        document = data.Document(text="Sample text.", additional_context=self._ADDITIONAL_CONTEXT)
        chunk_iter = chunking.ChunkIterator(
    ...
    def test_multiple_chunks_with_additional_context(self):
        ...
        document = data.Document(text=text, additional_context=self._ADDITIONAL_CONTEXT)
        chunk_iter = chunking.ChunkIterator(
            text=document.tokenized_text,
            max_char_buffer=15,
            document=document,
        ...
        chunks = list(chunk_iter)
        self.assertGreater(len(chunks), 1, "Should create multiple chunks with small buffer")
...
class TextChunkPropertyTest(parameterized.TestCase):
    ...
    def test_text_chunk_properties(self, document, expected_id, expected_text, expected_context):
