...
from langextract import factory, schema
...
class FactorySchemaIntegrationTest(absltest.TestCase):
    ...
    def test_no_schema_defaults_to_true_fence(self):
        ...
        class NoSchemaModel(base_model.BaseLanguageModel):
...
class SchemaApplicationTest(absltest.TestCase):
    ...
    def test_apply_schema_called_when_supported(self):
        ...
        examples = [
            data.ExampleData(
                text="Test",
                extractions=[data.Extraction(extraction_class="test", extraction_text="test")],
            )
        ]
        ...
        with (
            mock.patch(
                "langextract.providers.router.resolve",
                return_value=SchemaAwareModel,
            ),
            mock.patch.object(SchemaAwareModel, "__init__", return_value=None),
            mock.patch.object(SchemaAwareModel, "apply_schema") as mock_apply,
        ):
            _ = factory._create_model_with_schema(
                config=config,
                examples=examples,
