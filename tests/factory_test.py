...
from langextract import exceptions, factory
...
class FactoryTest(absltest.TestCase):
    def setUp(self):
        super().setUp()
        router.clear()
        import langextract.providers as providers_module
    ...
    def test_create_model_basic(self):
        ...
        config = factory.ModelConfig(model_id="gemini-pro", provider_kwargs={"api_key": "test-key"})

        model = factory.create_model(config)
        self.assertIsInstance(model, FakeGeminiProvider)
        assert isinstance(model, FakeGeminiProvider)
    ...
    def test_uses_openai_api_key_from_environment(self):
        ...
        model = factory.create_model(config)
        assert isinstance(model, FakeOpenAIProvider)
    ...
    def test_explicit_kwargs_override_env(self):
        ...
        with mock.patch.dict(os.environ, {"GEMINI_API_KEY": "env-key"}):
            config = factory.ModelConfig(
                model_id="gemini-pro",
                provider_kwargs={"api_key": "explicit-key"},
            )
    ...
    def test_wraps_provider_initialization_error_in_inference_config_error(
        self,
    ):
    ...
    def test_ollama_uses_base_url_from_environment(self):
        ...
        assert isinstance(model, FakeOllamaProvider)
    ...
    def test_uses_highest_priority_provider_when_multiple_match(self):
        ...
        class AnotherGeminiProvider(base_model.BaseLanguageModel):
    ...
    def test_explicit_provider_overrides_pattern_matching(self):
        ...
        assert isinstance(model, AnotherProvider)
        self.assertEqual(model.model_id, "gemini-pro")
    ...
    def test_provider_without_model_id_uses_provider_default(self):
        ...
        assert isinstance(model, DefaultProvider)
    ...
    def test_raises_error_when_neither_model_id_nor_provider_specified(self):
        ...
        self.assertIn("Either model_id or provider must be specified", str(cm.exception))

    def test_gemini_vertexai_parameters_accepted(self):
        """Test that Vertex AI parameters are properly passed to Gemini provider."""
        original_entries = router._entries.copy()
        original_keys = router._entry_keys.copy()
        ...
        try:
            ...
            assert isinstance(model, MockGeminiWithVertexAI)
            ...
            router._entries = original_entries
            router._entry_keys = original_keys
    ...
    def test_gemini_vertexai_with_credentials(self):
        ...
        try:
            ...
            assert isinstance(model, MockGeminiWithCredentials)
