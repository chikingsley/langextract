...
class InitTest(parameterized.TestCase):
    ...
    @mock.patch.object(schemas.gemini.GeminiSchema, "from_examples", autospec=True)
    @mock.patch("langextract.extraction.factory.create_model")
    def test_lang_extract_as_lx_extract(self, mock_create_model, mock_gemini_schema):
    ...
    @mock.patch("langextract.extraction.factory.create_model")
    def test_extract_resolver_params_alignment_passthrough(self, mock_create_model, mock_align):
        mock_model = mock.MagicMock()
        mock_model.infer.return_value = [[types.ScoredOutput(output='{"extractions":[]}')]]
    ...
    def test_extract_resolver_params_suppress_parse_errors(self, mock_create_model, mock_annotate):
        ...
        mock_annotate.return_value = lx.data.AnnotatedDocument(text="test", extractions=[])
    ...
    def test_extract_resolver_params_none_handling(self, mock_create_model, mock_resolver_class):
        ...
        with mock.patch("langextract.annotation.Annotator.annotate_text") as mock_annotate:
            mock_annotate.return_value = lx.data.AnnotatedDocument(text="test", extractions=[])
    ...
    def test_extract_resolver_params_none_threshold(
        ...
        mock_annotate.return_value = lx.data.AnnotatedDocument(text="t", extractions=[])
    ...
    @mock.patch.object(schemas.gemini.GeminiSchema, "from_examples", autospec=True)
    @mock.patch("langextract.extraction.factory.create_model")
    def test_extract_custom_params_reach_inference(self, mock_create_model, mock_gemini_schema):
        ...
        for name in expected_exports:
            with self.subTest(export=name):
                self.assertTrue(
                    ...
                    f"lx.tokenizer.{name} not accessible",
                )
    ...
    def test_show_progress_controls_progress_bar(
        ...
        self.assertEqual(call_args.kwargs.get("disable", False), expected_progress_disabled)
    ...
    def test_schema_validation_warning_issued(self, mock_create_model):
        ...
        mock_schema.validate_format = mock.Mock(side_effect=validate_format_side_effect)
    ...
    def test_gemini_schema_param_is_forwarded(self):
        """Test that passing gemini_schema remains a valid provider parameter."""
        mock_model = mock.MagicMock(spec=base_model.BaseLanguageModel)
        mock_model.infer.return_value = iter([[mock.Mock(output='{"extractions": []}')]])
        mock_model.requires_fence_output = True
        mock_model.schema = None

        mock_create_model = self.enter_context(
            mock.patch(
        ...
        _ = lx.extract(
            ...
            api_key="test_key",
            language_model_params={"gemini_schema": "active"},
        )

        create_call_kwargs = mock_create_model.call_args.kwargs
        self.assertIn("config", create_call_kwargs)
        self.assertIn(
            "gemini_schema",
            create_call_kwargs["config"].provider_kwargs,
        )
