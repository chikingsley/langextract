...
class PluginSmokeTest(absltest.TestCase):
    ...
    def test_plugin_discovery_and_usage(self):
        ...
        def _ep_load():
            ...
            class PluginProvider(base_model.BaseLanguageModel):
        ...
        with mock.patch.object(
            metadata,
            "entry_points",
            return_value=_create_mock_entry_points([ep]),
        ):
    ...
    def test_plugin_disabled_by_env_var(self):
        ...
        with (
            mock.patch.dict("os.environ", {"LANGEXTRACT_DISABLE_PLUGINS": "1"}),
            mock.patch.object(metadata, "entry_points") as mock_ep,
        ):
            lx.providers.load_plugins_once()
    ...
    def test_load_plugins_once_is_idempotent(self):
        ...
        def _ep_load():
            @lx.providers.router.register(r"^plugin-model")
            class Plugin(base_model.BaseLanguageModel):
                def infer(self, *a, **k):
                    return [[types.ScoredOutput(score=1.0, output="ok")]]
    ...
    def test_plugin_priority_override_core_provider(self):
        ...
        def _ep_load():
            ...
            class OverrideGemini(base_model.BaseLanguageModel):
                def infer(self, batch_prompts, **kwargs):
    ...
    def test_resolve_provider_for_plugin(self):
        ...
        def _ep_load():
            ...
            class ResolveMePlease(base_model.BaseLanguageModel):
                def infer(self, batch_prompts, **kwargs):
                    return [[types.ScoredOutput(score=1.0, output="ok")]]
    ...
    def test_plugin_with_custom_schema(self):
        ...
        def _ep_load():
            ...
            class SchemaTestProvider(base_model.BaseLanguageModel):
                ...
                def infer(self, batch_prompts, **kwargs):
                    output = f"Schema={self.schema_config}" if self.schema_config else "No schema"
...
class PluginE2ETest(absltest.TestCase):
    ...
    def test_plugin_install_discovery_and_cleanup(self):
        """Test complete plugin lifecycle: install, discovery, usage, uninstall.

        This test:
        1. Creates a Python package with a provider plugin
        2. Installs it via uv pip (falls back to pip)
        3. Verifies the plugin is discovered and usable
        4. Uninstalls and verifies cleanup
        """

        # Skip in Bazel environment where install operations don't work
        if os.environ.get("TEST_TMPDIR") or os.environ.get("BUILD_WORKING_DIRECTORY"):
            self.skipTest("install tests don't work in Bazel sandbox")
        ...
        if shutil.which("uv"):
            ...
            try:
                ...
                self.skipTest("Neither uv nor pip available in test environment")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (pkg_dir / pkg_name / "provider.py").write_text(
                textwrap.dedent("""
        import langextract as lx
      ...
      """)
            ...
            result = subprocess.run(
                [*install_cmd, "-e", str(pkg_dir), "--no-deps", "-q"],
                check=True,
                capture_output=True,
                text=True,
                env=install_env,
            ...
            self.assertEqual(result.returncode, 0, "Plugin install failed")
            self.assertNotIn(
                "ERROR",
                result.stderr.upper(),
                f"Plugin install had errors: {result.stderr}",
            )
          ...
          assert "with_schema" in result2[0][0].output, (
              f"Got: {{result2[0][0].output}}"
          )
          assert model2.requires_fence_output == False, (
              "Schema outputs raw JSON, should not need fences"
          )
          ...
          assert schema_cls.__name__ == "TestPipSchema", (
              "Schema class should be TestPipSchema, "
