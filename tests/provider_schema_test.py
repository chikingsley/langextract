...
from langextract import exceptions, factory, schema
from langextract.core import data
from langextract.providers import gemini, ollama, openai, schemas
...
class FormatModeSchemaTest(absltest.TestCase):
    ...
    def test_different_examples_same_output(self):
        ...
        examples1 = [
            data.ExampleData(
                text="Text 1",
                extractions=[data.Extraction(extraction_class="class1", extraction_text="text1")],
            )
        ]
...
class OllamaFormatParameterTest(absltest.TestCase):
    ...
    def test_ollama_json_format_in_request_payload(self):
        ...
        with mock.patch("langextract.providers.ollama.httpx.post", autospec=True) as mock_post:
            ...
            self.assertEqual(payload["model"], "test-model", msg="Model ID should match")
            self.assertEqual(payload["prompt"], "Test prompt", msg="Prompt should match")
    ...
    def test_ollama_default_format_is_json(self):
        ...
        with mock.patch("langextract.providers.ollama.httpx.post", autospec=True) as mock_post:
            ...
            self.assertEqual(payload["format"], "json", msg="Default format should be json")
    ...
    def test_extract_with_ollama_passes_json_format(self):
        ...
        with mock.patch("langextract.providers.ollama.httpx.post", autospec=True) as mock_post:
            ...
            mock_response.json.return_value = {
                "response": (
                    '{"extractions": [{"extraction_class": "test", "extraction_text": "example"}]}'
                )
            }
            mock_post.return_value = mock_response

            # Mock the registry to return OllamaLanguageModel
            with mock.patch("langextract.providers.router.resolve") as mock_resolve:
                ...
                self.assertEqual(payload["model"], "gemma2:2b", msg="Model ID should match")
...
class OllamaYAMLOverrideTest(absltest.TestCase):
    """Tests for Ollama YAML mode behavior."""

    def test_ollama_yaml_does_not_force_structured_format(self):
        """YAML mode should not force Ollama JSON-format payload setting."""
        with mock.patch("langextract.providers.ollama.httpx.post", autospec=True) as mock_post:
            mock_response = mock.Mock(spec=["status_code", "json"])
            mock_response.status_code = 200
            ...
            mock_post.assert_called_once()
            call_kwargs = mock_post.call_args[1]
            self.assertIn("json", call_kwargs, msg="Request should use json parameter")
            payload = call_kwargs["json"]
            self.assertNotIn("format", payload, msg="YAML mode should not set format payload key")
        ...
        with mock.patch("langextract.providers.ollama.httpx.post", autospec=True) as mock_post:
            ...
            with mock.patch("langextract.providers.router.resolve") as mock_resolve:
                ...
                self.assertTrue(
                    ...
                    msg="YAML format should require fences",
                )
...
class SchemaShimTest(absltest.TestCase):
    """Tests for public imports in schema module."""
    ...
    def test_provider_schema_imports(self):
        ...
        self.assertTrue(
            ...
            msg=("GeminiSchema should be importable from schema module"),
        )
