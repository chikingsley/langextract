...
from langextract.providers import gemini, schemas
...
def create_mock_batch_job(
    ...
    job.state = state
    job.dest = mock.create_autospec(genai.types.BatchJobDestination, instance=True)
    ...
    return json.dumps(
        {
            ...
            "response": {"candidates": [{"content": {"parts": [{"text": text_content}]}}]},
        }
    )
...
class GCSBatchCachingTest(absltest.TestCase):
    ...
    def test_partial_cache_hit(self, mock_client_cls):
        ...
        with mock.patch.object(gb.GCSBatchCache, "_compute_hash") as mock_hash:
            ...
            self.assertTrue(upload_calls, "Should have uploaded new_response to cache")
    ...
    def test_project_passed_to_storage_client(self, mock_client_cls):
        ...
        project_calls = [
            call for call in storage_calls if call.kwargs.get("project") == test_project
        ]
