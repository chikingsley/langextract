...
from langextract.providers import ollama, openai
...
class TestOpenAIKwargsPassthrough(unittest.TestCase):
    ...
    def test_reasoning_dict_pass_through(self, mock_openai_class):
        """Reasoning config should pass through unchanged."""
        mock_client = mock.Mock()
        mock_openai_class.return_value = mock_client

        mock_response = mock.Mock()
        mock_response.choices = [mock.Mock(message=mock.Mock(content='{"result": "test"}'))]
    ...
    def test_reasoning_parameter_normalized(self, mock_openai_class):
        """Runtime reasoning dict should pass through."""
        mock_client = mock.Mock()
        mock_openai_class.return_value = mock_client
        ...
        list(model.infer(["test prompt"], reasoning={"effort": "maximal"}))
    ...
    @mock.patch("openai.OpenAI")
    def test_reasoning_override_runtime(self, mock_openai_class):
        """Runtime reasoning should override constructor reasoning."""
        mock_client = mock.Mock()
        mock_openai_class.return_value = mock_client
        ...
        model = openai.OpenAILanguageModel(
            model_id="gpt-5",
            api_key="test-key",
            reasoning={"other_field": "value", "effort": "maximal"},
        )

        list(model.infer(["test prompt"], reasoning={"effort": "low"}))

        call_args = mock_client.chat.completions.create.call_args
        self.assertEqual(call_args.kwargs.get("reasoning"), {"effort": "low"})
...
class TestOllamaAuthSupport(parameterized.TestCase):
    ...
    @mock.patch("langextract.providers.ollama.httpx.post")
    ...
    def test_api_key_redacted_in_repr(self):
        ...
        self.assertNotIn("super-secret-key", repr_str, "Actual API key should not appear")
    ...
    def test_runtime_kwargs_override(self, mock_post):
        ...
        self.assertEqual(call_args.kwargs.get("timeout"), 120.0)
    ...
    def test_reserved_kwargs_not_in_options(self, mock_post):
        ...
        self.assertNotIn("stop", options, "stop should be at top level, not in options")
        self.assertEqual(options.get("temperature"), 0.5)
    ...
    def test_api_key_without_localhost_warning(self, mock_post):
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertFalse(any("localhost" in str(warning.message) for warning in w))
