...
skip_if_no_vertex = pytest.mark.skipif(
    ...
    reason=(
        "Vertex AI credentials not available (set GOOGLE_CLOUD_PROJECT and configure gcloud auth)"
    ),
)
...
def retry_on_transient_errors(max_retries=3, backoff_factor=2.0):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            raise RuntimeError("No exception captured during retry sequence")
...
def get_basic_medication_examples():
    ...
    return [
        lx.data.ExampleData(
            ...
            extractions=[
                lx.data.Extraction(extraction_class=_CLASS_DOSAGE, extraction_text="250 mg"),
                lx.data.Extraction(extraction_class=_CLASS_ROUTE, extraction_text="IV"),
                lx.data.Extraction(
...
def extract_by_class(result, extraction_class):
    ...
    """
    return {e.extraction_text for e in result.extractions if e.extraction_class == extraction_class}
...
def assert_valid_char_intervals(test_case, result):
    ...
    for extraction in result.extractions:
        ...
        test_case.assertIsNotNone(
            ...
            f"Missing alignment_status for extraction: {extraction.extraction_text}",
        )
...
class TestLiveAPIGemini(unittest.TestCase):
    ...
    def _check_cached_result(self, result_json: dict[str, Any]) -> bool:
        ...
        try:
            ...
            doc = data.AnnotatedDocument(text=inner_json.get("text"), extractions=extractions)
    ...
    def test_medication_extraction(self):
        ...
        self.assertTrue(
            any(re.search(r"\bIbuprofen\b", text, re.IGNORECASE) for text in medication_texts),
            f"No Ibuprofen found in: {medication_texts}",
        )

        dosage_texts = extract_by_class(result, _CLASS_DOSAGE)
        self.assertTrue(
            any(re.search(r"\b400\s*mg\b", text, re.IGNORECASE) for text in dosage_texts),
            f"No 400mg dosage found in: {dosage_texts}",
        )

        route_texts = extract_by_class(result, _CLASS_ROUTE)
        self.assertTrue(
            any(re.search(r"\b(PO|oral)\b", text, re.IGNORECASE) for text in route_texts),
    ...
    def test_multilingual_medication_extraction(self):
        ...
        assert len(medication_extractions) > 0, "No medication entities found in Japanese text"
        assert_valid_char_intervals(self, result)
    ...
    """)
        ...
        for extraction in result.extractions:
            ...
            assert "medication_group" in extraction.attributes, (
        ...
        assert len(medication_groups) >= 2, (
        ...
        for med_name, extractions in medication_groups.items():
            ...
            assert _CLASS_MEDICATION in extraction_classes, (
                f"{med_name} group missing medication entity"
            )
            # Dosage is expected but might be formatted differently
            assert any(c in extraction_classes for c in [_CLASS_DOSAGE, "dose"]), (
    ...
    def test_batch_extraction_vertex_gcs(self, mock_infer_batch):
        ...
        for i, (res, med_name) in enumerate(zip(batch_result, expected_meds, strict=True)):
            ...
            for extraction in res.extractions:
                self.assertIsInstance(
                    extraction,
                    lx.data.Extraction,
                    f"Extraction item should be Extraction object, got {type(extraction)}",
                )

            meds = extract_by_class(res, _CLASS_MEDICATION)
            self.assertTrue(
                any(re.search(rf"\b{re.escape(med_name)}\b", m, re.IGNORECASE) for m in meds),
...
class TestCrossChunkContext(unittest.TestCase):
    ...
    def test_context_window_extracts_from_both_chunks(self):
        ...
        all_extraction_text = " ".join(
            str(e.extraction_text) + " " + str(e.attributes) for e in result.extractions
        ).lower()

        has_chunk1_content = any(
            term in all_extraction_text for term in ("sarah", "chen", "researcher", "lead")
        )
