...
class TokenizerTest(parameterized.TestCase):
    def assertTokenListEqual(self, actual_tokens, expected_tokens, msg=None):
        ...
        for i, (expected, actual) in enumerate(zip(expected_tokens, actual_tokens, strict=True)):
...
class UnicodeTokenizerTest(parameterized.TestCase):
    ...
    def test_special_unicode_and_punctuation_handling(
        ...
        for i, (
            ...
            actual_text = input_text[token.char_interval.start_pos : token.char_interval.end_pos]
            self.assertEqual(
                actual_text,
                expected_text,
    ...
    def test_script_boundary_grouping(self):
        ...
        t1_text = text[
            tokenized.tokens[0].char_interval.start_pos : tokenized.tokens[0].char_interval.end_pos
        ]
        t2_text = text[
            tokenized.tokens[1].char_interval.start_pos : tokenized.tokens[1].char_interval.end_pos
        ]
    ...
    def test_non_spaced_scripts_no_grouping(self):
        ...
        self.assertGreater(
            ...
            1,
            "Should not be grouped into a single token",
        )
    ...
    def test_consecutive_punctuation_grouping(self):
        ...
        self.assertEqual(
            [input_text[t.char_interval.start_pos : t.char_interval.end_pos] for t in tokens],
            expected_tokens,
        )
    ...
    def test_punctuation_merging_identical_only(self):
        ...
        tokens_mixed = tokenizer.UnicodeTokenizer().tokenize(input_text_mixed).tokens
        self.assertEqual(
            [
                input_text_mixed[t.char_interval.start_pos : t.char_interval.end_pos]
...
class ExceptionTest(absltest.TestCase):
    ...
    def test_invalid_token_interval_errors(self):
        ...
        with self.assertRaisesRegex(
            ...
            tokenizer.tokens_text(tokenized, tokenizer.TokenInterval(start_index=-1, end_index=1))

        with self.assertRaisesRegex(
            tokenizer.InvalidTokenIntervalError,
            "Invalid token interval.*end_index=999",
        ):
            tokenizer.tokens_text(tokenized, tokenizer.TokenInterval(start_index=0, end_index=999))

        with self.assertRaisesRegex(
            tokenizer.InvalidTokenIntervalError,
            "Invalid token interval.*start_index=2.*end_index=1",
        ):
            tokenizer.tokens_text(tokenized, tokenizer.TokenInterval(start_index=2, end_index=1))
...
class NegativeTestCases(parameterized.TestCase):
    ...
    def test_invalid_and_edge_case_unicode(self, input_text, expected_tokens):
        ...
        for i, (token, (expected_text, expected_type)) in enumerate(
            zip(tokenized.tokens, expected_tokens, strict=True)
        ):
            ...
            self.assertEqual(
                actual_text,
                expected_text,
                f"Token {i} text mismatch. Expected '{expected_text}', got '{actual_text}'",
            )
            self.assertEqual(
                token.token_type,
                expected_type,
                f"Token {i} type mismatch. Expected {expected_type}, got {token.token_type}",
            )
    ...
    def test_empty_string_edge_case(self):
        ...
        self.assertEqual(tokenized.text, "", "Tokenized text should preserve empty string")
    ...
    def test_whitespace_only_string(self):
        ...
        for whitespace in test_cases:
            ...
            self.assertEmpty(
                tokenized.tokens,
                f"Whitespace-only string '{whitespace!r}' should produce no tokens",
            )
...
class TokensTextTest(parameterized.TestCase):
    ...
    def test_valid_intervals(self, input_text, start_index, end_index, expected_substring):
        ...
        interval = tokenizer.TokenInterval(start_index=start_index, end_index=end_index)
...
class SentenceRangeTest(parameterized.TestCase):
    ...
    def test_configurable_sentence_splitting(self):
        ...
        sentence1 = tokenizer.find_sentence_range(text_french, tokenized_french.tokens, 0)
