...
class VisualizationTest(absltest.TestCase):
    ...
    def test_build_highlighted_text_single_span_correct_html(self):
        ...
        actual_html = visualization._build_highlighted_text(text, extractions, color_map)
    ...
    @mock.patch.object(visualization, "HTML", new=None)  # Ensures visualize returns str
        ...
        doc = data.AnnotatedDocument(
            ...
            extractions=[
                data.Extraction(
                    ...
                    char_interval=data.CharInterval(start_pos=14, end_pos=21),  # "Aspirin"
                )
            ],
        )
    ...
    def test_visualize_no_extractions_renders_text_and_empty_legend(self):
        ...
        body_html = '<div class="lx-animated-wrapper"><p>No valid extractions to animate.</p></div>'
        css_html = _VISUALIZATION_CSS
